...
class UnShuffle_Layer(nn.Module):
    ...
    def forward(self, x):
        ...
        out_channels = self.ratios * channels
...
class HRM_Block(nn.Module):
    def __init__(self, in_planes = 96, stride = 2, kernel_size=3):
        ...
        self.horizontal_convbn = ConvBN(in_planes=in_planes, out_planes=in_planes, kernel_size=1)
        self.unshuffle_layer = UnShuffle_Layer(ratios=stride)
        padding = (kernel_size - 1) // 2
        self.unshuffle_convbn = ConvBN(in_planes=in_planes*stride, out_planes=in_planes, kernel_size=kernel_size, padding=padding)
